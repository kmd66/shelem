...
public class RoomHub(RoomService roomService) : Hub
{
    ...
    public void Start(Guid roomId, Guid key)
    {
        ...
        {
            var room = _roomService.GetRoom(roomId);
            if (room == null || room._p.IsStart) return;
            //room._p.SetStart(key);
        }
        catch (Exception)
        {
        };
    ...
    public async Task AddSticker(Guid roomId, Guid key, byte id)
    {
        try
        {
            var room = _roomService.GetRoom(roomId);
            if (room == null) return;
            var user = room._p.Users.FirstOrDefault(x => x.Key == key);
            if (user == null) return;
            await Clients.Clients(room._p.Users.Select(x => x.ConnectionId)).SendAsync("ReceiveAddSticker", new { id, userId = user.Id });
        }
        catch (Exception)
        {
        };
    ...
    public async Task AddMessage(Guid roomId, Guid key, byte id)
    {
        try
        {
            ...
            var user = room._p.Users.FirstOrDefault(x => x.Key == key);
            if (user == null) return;
            await Clients.Clients(room._p.Users.Select(x => x.ConnectionId)).SendAsync("ReceiveAddMessage", new { id, userId = user.Id });
        }
        catch (Exception)
        {
        };
